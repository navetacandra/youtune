  ...
  return runs.map((r: any) => {
    ...
    const type: string = r.navigationEndpoint?.watchEndpoint 
      ...
      : r.navigationEndpoint?.browseEndpoint.browseEndpointContextSupportedConfigs
        .browseEndpointContextMusicConfig.pageType.replace('MUSIC_PAGE_TYPE_', '').toLowerCase();
    const id: string = r.navigationEndpoint?.watchEndpoint?.videoId || r.navigationEndpoint?.browseEndpoint?.browseId;
    return {id, text, type};
  })
}
