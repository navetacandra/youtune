...
export async function GET() {
  ...
  const contents: Content[] = res.contents.singleColumnBrowseResultsRenderer.tabs[0]
  .tabRenderer.content.sectionListRenderer.contents
  .filter((c: {[k: string]: boolean}) => c.musicCarouselShelfRenderer)
  .map((_c: any) => {
    const c: any = _c.musicCarouselShelfRenderer;
    const category: string = c.header.musicCarouselShelfBasicHeaderRenderer.title.runs[0].text
    const contents = c.contents.map((item: any) => {
      ...
      if(item.musicTwoRowItemRenderer) {
        ...
        title: TextComponentDetail = parseTitle(mtrir.title.runs.filter((f: boolean) => f))[0],
        subtitle: TextComponentDetail[] = parseTitle(mtrir.subtitle.runs.filter((f: boolean) => f)),
        _thumbnail: string = mtrir.thumbnailRenderer.musicThumbnailRenderer.thumbnail.thumbnails.slice(-1)[0].url,
        thumbnail: string = _thumbnail.match(/https:\/\/i\.ytimg\.com/i) ? _thumbnail : _thumbnail.replace(/=w\d+-h\d+/, '=w512-h512');
        return {id: title.id, type: title.type, thumbnail, title, subtitle};
      }
      return null
