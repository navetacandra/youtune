...
export async function GET(
  ...
  try {
    ...
    const content =
      res.contents.tabbedSearchResultsRenderer.tabs[0].tabRenderer.content
        .sectionListRenderer.contents;

    const headContent: ContentDetail[] = content
      .filter((c: { [k: string]: boolean }) => c.musicCardShelfRenderer)
      .map((c: { [k: string]: any }) => {
        const mcsr: any = c.musicCardShelfRenderer,
          ...
          _thumbnail: string =
            mcsr.thumbnail.musicThumbnailRenderer.thumbnail.thumbnails.slice(
              -1,
            )[0].url,
          thumbnail: string = _thumbnail.match(/https:\/\/i\.ytimg\.com/i)
            ? _thumbnail
            : _thumbnail.replace(/=w\d+-h\d+/, "=w512-h512");
        return { id: title?.id, type: title?.type, thumbnail, title, subtitle };
      });

    const _mcontents = content.find(
      (c: { [k: string]: boolean }) => c.musicCardShelfRenderer,
    ...
    const _contents: Content[] = content
      .filter(
        (c: { [k: string]: boolean }) =>
          !c.musicCardShelfRenderer && c.musicShelfRenderer,
      )
      .map((c: { [k: string]: any }) => {
