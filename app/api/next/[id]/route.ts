...
export const GET = async (
  ...
  try {
    ...
    const automix = mqr.content.playlistPanelRenderer.contents.find((c: any) => c.automixPreviewVideoRenderer).automixPreviewVideoRenderer;
    const endpoint = automix.content.automixPlaylistVideoRenderer.navigationEndpoint.watchPlaylistEndpoint;

    const next = await fapi('next', { ...args, ...endpoint });
    const contents = next.contents.singleColumnMusicWatchNextResultsRenderer.tabbedRenderer.watchNextTabbedResultsRenderer.tabs[0].tabRenderer.content.musicQueueRenderer.content.playlistPanelRenderer.contents.map((c: any) => c.playlistPanelVideoRenderer);
    const data = contents.map((c: any) => {
      ...
      thumbnail = thumbnail.match(/i\.ytimg\.com/) ? thumbnail.split('?')[0] : thumbnail.replace(/=w\d+-h\d+/, "=w512-h512");
